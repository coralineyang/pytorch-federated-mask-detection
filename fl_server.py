...
class FLServer(object):
    def __init__(self, task_config_filename, host, port):
        ...
        self.socketio = SocketIO(self.app, ping_timeout=1000000,
        ...
        # 定义log的形式 当前时间-logger名字-文本形式的日志级别-用户输出的消息
    ...
    def register_handles(self):
        ...
        def handle_disconnect():
            ...
            for client_num, sid in self.ready_client_sids.items():
                if sid == request.sid:
                    del self.ready_client_sids[client_num]
        ...
        def handle_client_ready(data):
            ...
            self.ready_client_sids[data['client_number']]=request.sid
            print(self.ready_client_sids,self.MIN_NUM_WORKERS )
        ...
        def handle_client_update(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                if len(self.current_round_client_updates) == self.NUM_CLIENTS_CONTACTED_PER_ROUND: #全部weight都给了server
        ...
        def handle_client_eval(data):
            ...
            print(self.eval_client_updates)
    ...
    def check_client_resource(self):
        ...
        self.client_resource = {}
        client_sids_selected = random.sample(list(self.ready_client_sids.values()), self.NUM_CLIENTS_CONTACTED_PER_ROUND)
        print('send weights')
        print(client_sids_selected)
    ...
    def train_next_round(self, client_sids_selected):
        ...
        print('request_update,', client_sids_selected)
        for rid in client_sids_selected:
    ...
    def stop_and_eval(self):
        current_weights = obj_to_pickle_string(self.aggregator.current_weights, self.aggregator.model_path)
        self.eval_client_updates = []
        print('stop_and_eval:', self.ready_client_sids)
        for rid in self.ready_client_sids.values():
            emit('stop_and_eval', {
                'model_id': self.model_id,
                #'current_weights': current_weights,
            ...
            self.logger.info("sent aggregated model to client {}".format(rid))
        # emit('stop_and_eval', {
        #     'model_id': self.model_id,
        #     # 'current_weights': current_weights,
        #     'weights_format': 'pickle',
        #     'STOP': self.STOP
        # }, room=request.sid)
        # self.logger.info("sent aggregated model to client {}".format(request.sid))
...
if __name__ == '__main__':
    config_file = "data/task_configs/yolo/clients_data/yolo_task.json"
    port = '1234'
    if not os.path.exists(config_file):
        raise FileNotFoundError("{} dose not exist".format(config_file))
    try:
        server = FLServer(config_file, "127.0.0.1", port)
        print("listening on 127.0.0.1:{}".format(str(port)))
        server.start()
